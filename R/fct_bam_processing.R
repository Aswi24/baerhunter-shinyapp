...
#'
#' @param bam_files BAM files to be analysed
...
#'
#'
peak_union_calc <- function(bam_files, target_strand, low_coverage_cutoff, high_coverage_cutoff,  peak_width, paired_end_data = FALSE, strandedness  = "unstranded") {
  ...
  for (f in bam_files) {
    ...
    peak_union <- IRanges::union(peak_union,peaks_IRange)
    print("peak union done")
  }
  return(peak_union)
}
...
#' @param peak_width An integer indicating the minimum sRNA length (peak width).
#'
#' @return Returns a RleViews line if it satisfies conditions.
#'
#' @export
#'
peak_analysis <- function(View_line, high_cutoff,peak_width) {
  ## This is a helper function that is used to examine if the peak had a continuous stretch of a given width that has coverage above the high cut-off value.
  cvg_string <- as.vector(View_line)
  target_peak <- which(cvg_string>high_cutoff)
  if (length(target_peak)>peak_width) {
    return(View_line)
  }
}
...
#' @export
major_features <- function(annotation_file, target_strand, original_sRNA_annotation) {


  gff <- read.delim(annotation_file, header = FALSE, comment.char = "#")
...
sRNA_calc <- function(major_strand_features, target_strand, union_peak_ranges) {
  ...
  IGR_sRNAs <- union_peak_ranges[match(union_peak_ranges, IRanges::subsetByOverlaps(union_peak_ranges, strand_IRange, maxgap = 1L)) == 0,]
...
UTR_calc <- function(major_strand_features, target_strand, union_peak_ranges, min_UTR_length) {
  ...
  UTRs <- split_features[match(split_features, IRanges::subsetByOverlaps(split_features, strand_IRange)) == 0]
...
#' @param low_coverage_cutoff An integer indicating the low coverage threshold value.
#' @param high_coverage_cutoff An integer indicating the high coverage threshold value.
#' @param peak_width An integer indicating the minimum peak width/sRNA length.
#' @param min_UTR_length An integer indicating the minimum UTR length.
#' @param paired_end_data A boolean indicating if the reads are paired-end.
#' @param strandedness A string outlining the type of the sequencing library: stranded, or reversely stranded.
#'
...
#'
#' @export
feature_file_editor <- function(bam_files, original_annotation_file, original_sRNA_annotation, output_file, low_coverage_cutoff, high_coverage_cutoff, peak_width, min_UTR_length, paired_end_data = FALSE, strandedness  = "stranded", progressCallback = NULL) {

  num_files <- length(bam_files)
  for (i in seq_along(bam_files)) {
    bam_file <- bam_files[i]

    # Calculate and update progress at the start of processing each file
    if (!is.null(progressCallback)) {
      progress_val <- (i - 1) / num_files
      progress_msg <- sprintf("Processing %s: plus strand data", basename(bam_file))
      progressCallback(progress_val, progress_msg)
    }

    plus_strand_peaks <- peak_union_calc(bam_files, "+", low_coverage_cutoff, high_coverage_cutoff,  peak_width, paired_end_data, strandedness)

    if (!is.null(progressCallback)) {
      progressCallback(progress_val + 0.5/num_files, sprintf("Building plus strand annotation dataframe for %s", basename(bam_file)))
    }

    print("Extracted plus strand data from BAM files")
    maj_plus_features <- major_features(original_annotation_file, "+", original_sRNA_annotation)
    plus_sRNA <- sRNA_calc(maj_plus_features, "+", plus_strand_peaks)
    plus_UTR <- UTR_calc(maj_plus_features, "+", plus_strand_peaks, peak_width)
    ...
    ## Minus strand
    minus_strand_peaks <- peak_union_calc(bam_files,"-", low_coverage_cutoff, high_coverage_cutoff, peak_width, paired_end_data, strandedness)
    print("Extracted minus strand data from BAM files")
    maj_minus_features <- major_features(original_annotation_file, "-", original_sRNA_annotation)
    ...
    if (!is.null(progressCallback)) {
      progressCallback((i / num_files), sprintf("Completed processing %s", basename(bam_file)))
    }
  }

  gff <- read.delim(original_annotation_file, header = FALSE, comment.char = "#")
  ...
  f <- readLines(original_annotation_file)
  header <- c()
  i <- 1
  ...
  if (!is.null(progressCallback)) {
    progressCallback(1, "GFF file ready for download!")
  }
